...
#Divide and Conquer A, left half.
mid = len(D) // 2
D_left = D[:mid]

# Divide D into 10 equal parts
part_size = len(D_left) // 20
for j in range(20):
    # Get the slice of D for this part
    part_D = D_left[j*part_size : (j+1)*part_size]
    ...
    with open(f"/scratch/madhu/kritika/Depth/scoring_nbrs/distribution/Leftpart_{j}.pickle", "wb") as f:
        pickle.dump(Nrs, f)
